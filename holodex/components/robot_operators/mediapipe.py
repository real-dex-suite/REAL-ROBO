...
class MPDexArmTeleOp(object):
    def __init__(self):
        ...
        robohand_bounds_path = get_path_in_package(
            f"components/robot_operators/configs/{hand_type}_mp.yaml"
        )
    ...
    def motion_2D(self, finger_configs):
        ...
        if not finger_configs["freeze_index"]:
            desired_joint_angles = self.finger_tip_solver.finger_1D_motion(
                ...
                z_robot_bound=[
                    self.robohand_bounds["index"]["z_bottom"],
                    self.robohand_bounds["index"]["z_top"],
                ],
        ...
        if not finger_configs["freeze_middle"]:
            desired_joint_angles = self.finger_tip_solver.finger_1D_motion(
                ...
                y_hand_bound=self.hand_bounds[0],
                z_robot_bound=[
                    self.robohand_bounds["middle"]["z_bottom"],
                    self.robohand_bounds["middle"]["z_top"],
                ],
        ...
        desired_joint_angles = self.finger_tip_solver.finger_2D_motion(
            ...
            y_robot_bound=[
                self.robohand_bounds["ring"]["y_left"],
                self.robohand_bounds["ring"]["y_right"],
            ],
            z_robot_bound=[
                self.robohand_bounds["ring"]["z_bottom"],
                self.robohand_bounds["ring"]["z_top"],
            ],
    ...
    def motion_3D(self, finger_configs):
        ...
        if not finger_configs["freeze_index"]:
            desired_joint_angles = self.finger_tip_solver.finger_2D_depth_motion(
                ...
                x_robot_bound=[
                    self.robohand_bounds["index"]["x_bottom"],
                    self.robohand_bounds["index"]["x_top"],
                ],
                z_robot_bound=[
                    self.robohand_bounds["index"]["z_bottom"],
                    self.robohand_bounds["index"]["z_top"],
        ...
        if not finger_configs["freeze_middle"]:
            desired_joint_angles = self.finger_tip_solver.finger_2D_depth_motion(
                ...
                x_robot_bound=[
                    self.robohand_bounds["middle"]["x_bottom"],
                    self.robohand_bounds["middle"]["x_top"],
                ],
                z_robot_bound=[
                    self.robohand_bounds["middle"]["z_bottom"],
                    self.robohand_bounds["middle"]["z_top"],
        ...
        desired_joint_angles = self.finger_tip_solver.finger_3D_motion(
            ...
            x_robot_bound=[
                self.robohand_bounds["ring"]["x_bottom"],
                self.robohand_bounds["ring"]["x_top"],
            ],
            y_robot_bound=[
                self.robohand_bounds["ring"]["y_left"],
                self.robohand_bounds["ring"]["y_right"],
        ...
        if coord_in_bound(self.hand_bounds[7:11], finger_tip_coords["thumb"][:2]) > -1:
            desired_joint_angles = self.finger_tip_solver.thumb_motion_3D(
                ...
                x_robot_bound=[
                    self.robohand_bounds["thumb"]["x_bottom"],
                    self.robohand_bounds["thumb"]["x_top"],
                ],
                moving_avg_arr=self.moving_average_queues["thumb"],
                curr_angles=desired_joint_angles,
            )
