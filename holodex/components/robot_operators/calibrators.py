...
class MPHandBoundCalibrator(object):
    ...
    def _get_bounds(self, finger_type, coord_axis):
        ...
        print(
            "Getting the {} bounds for the {} finger.".format(coord_axis, finger_type)
        )
        bounds = np.zeros((1, 2))

        register = input(
            "Press Enter to register the upper {} bound for the {} finger.".format(
                coord_axis, finger_type
            )
        )
        ...
        register = input(
            "Press Enter to register the lower {} bound for the {} finger.".format(
                coord_axis, finger_type
            )
        )
    ...
    def _get_thumb_2d_bounds(self):
        ...
        register = input(
            "Press Enter to register the upper right bound for the Thumb finger."
        )
        ...
        register = input(
            "Press Enter to register the lower right bound for the Thumb finger."
        )
        ...
        register = input(
            "Press Enter to register the lower left bound for the Thumb finger."
        )
        ...
        register = input(
            "Press Enter to register the upper left bound for the Thumb finger."
        )
...
class OculusThumbBoundCalibrator(object):
    ...
    def _callback_keypoints(self, coord_data):
        self.hand_coords = np.array(list(coord_data.data)).reshape(
