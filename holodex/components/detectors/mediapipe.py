...
class MPHandDetector(object):
    def __init__(self, cam_serial_num, resolution, alpha):
        ...
        self.og_rgb_image_publisher = ImagePublisher(
            publisher_name=MP_RGB_IMAGE_TOPIC, color_image=True
        )
        self.og_depth_image_publisher = ImagePublisher(
            publisher_name=MP_DEPTH_IMAGE_TOPIC, color_image=False
        )
        self.pred_rgb_image_publisher = ImagePublisher(
            publisher_name=MP_PRED_RGB_IMAGE_TOPIC, color_image=True
        )
        self.pred_depth_image_publisher = ImagePublisher(
            publisher_name=MP_PRED_DEPTH_IMAGE_TOPIC, color_image=False
        )
    ...
    def _get_depth_value(self, x_pixel, y_pixel, depth_frame):
        ...
        for x_range in pixel_range:
            for y_range in pixel_range:
                ...
                if (
                    curr_x_pixel >= 0
                    and curr_x_pixel < self.resolution[0]
                    and curr_y_pixel >= 0
                    and curr_y_pixel < self.resolution[1]
                ):
    ...
    def get_keypoints(self, hand_landmarks, depth_frame):
        ...
        for idx, point in enumerate(hand_landmarks.landmark):
            ...
            if idx == 0:  # For the wrist
                ...
                x_pixel, y_pixel = int(self.resolution[0] * point.x), int(
                    self.resolution[1] * point.y
                )
                ...
                if new_depth > 0:
                    wrist_coords[2] = self._temporal_depth_average(
                        0, new_depth, alpha=self.alpha
                    )

                # Finding the x and y coordinates
                wrist_coords[0] = (
                    wrist_coords[2]
                    * (x_pixel - self.intrinsics_matrix[0][2])
                    / self.intrinsics_matrix[0][0]
                )
                wrist_coords[1] = (
                    wrist_coords[2]
                    * (y_pixel - self.intrinsics_matrix[1][2])
                    / self.intrinsics_matrix[1][1]
                )
            ...
            if idx in MP_OG_KNUCKLES:  # For the knuckles
                ...
                # Using the temporal moving average filter
                if new_depth > 0:
                    knuckle_coords[knuckle_counter][2] = self._temporal_depth_average(
                        knuckle_counter + 1, new_depth, alpha=self.alpha
                    )

                # Finding the x and y coordinates
                knuckle_coords[knuckle_counter][0] = (
                    knuckle_coords[knuckle_counter][2]
                    * (x_pixel - self.intrinsics_matrix[0][2])
                    / self.intrinsics_matrix[0][0]
                )
                knuckle_coords[knuckle_counter][1] = (
                    knuckle_coords[knuckle_counter][2]
                    * (y_pixel - self.intrinsics_matrix[1][2])
                    / self.intrinsics_matrix[1][1]
                )
            ...
            if idx in MP_OG_FINGERTIPS:  # For the tips
                ...
                # Using the temporal moving average filter
                if new_depth > 0:
                    tip_coords[tip_counter][2] = self._temporal_depth_average(
                        tip_counter + 6, new_depth, alpha=self.alpha
                    )

                # Finding the x and y coordinates
                tip_coords[tip_counter][0] = (
                    tip_coords[tip_counter][2]
                    * (x_pixel - self.intrinsics_matrix[0][2])
                    / self.intrinsics_matrix[0][0]
                )
                tip_coords[tip_counter][1] = (
                    tip_coords[tip_counter][2]
                    * (y_pixel - self.intrinsics_matrix[1][2])
                    / self.intrinsics_matrix[1][1]
                )
    ...
    def stream(self):
        with self._mediapipe_hands.Hands(
            ...
            min_tracking_confidence=PRED_CONFIDENCE,
        ) as hand:
            ...
            while True:
                ...
                if prediction.multi_hand_landmarks:
                    ...
                    obtained_keypoints = self.get_keypoints(
                        prediction.multi_hand_landmarks[0], depth_frame
                    )
