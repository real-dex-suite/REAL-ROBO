...
class DexArmDeploy(object):
    def __init__(self, deploy_configs):
        ...
        transform = T.Compose(
            [
                ...
                T.Normalize(
                    mean=torch.tensor(
                        self.configs["task"]["image_parameters"]["mean_tensors"][
                            self.cam_num - 1
                        ]
                    ),
                    std=torch.tensor(
                        self.configs["task"]["image_parameters"]["std_tensors"][
                            self.cam_num - 1
                        ]
                    ),
                ),
        ...
        if model == "VINN":
            ...
            self.model = DeployVINN(
                ...
                encoder_weights_path=self.configs["task"]["vinn"][
        ...
        self.robot = RobotController(
            teleop=False, servo_mode=True, arm_control_mode="joint"
        )
    ...
    def _transform_image(self, image):
        ...
        image = image.crop(
            self.configs["task"]["image_parameters"]["crop_sizes"][self.cam_num - 1]
        )
        image = image.resize(
            (
    ...
    def _postprocess_arm_action(self, pred_action, action_type):
        ...
        if action_type == "cartesian_pose":
            ...
            processed_action = np.array(
                converter.unscale_transform(
                    arm_joint, ARM_JOINT_LOWER_LIMIT, ARM_JOINT_UPPER_LIMIT
                )
            )
            return processed_action
        else:
    ...
    def _postprocess_hand_action(self, pred_action, action_type):
        ...
        ):  # TODO: quat, also have to consider the euler angle
        ...
        hand_joint = np.array(
            converter.unscale_transform(
                hand_joint, HAND_JOINT_LOWER_LIMIT, HAND_JOINT_UPPER_LIMIT
            )
        )
        return hand_joint
    ...
    def _create_robot_action(self, pred_action):
        ...
        return {"arm": arm_ee_pose, "hand": hand_joint}
