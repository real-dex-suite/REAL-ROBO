...
class DeployVINN(object):
    def __init__(
        ...
        self.nn_extractor = KNearestNeighbors(
            self._input_representations, self._actions
        )
        ...
        (
            ...
            self.cumm_len,
            self.traj_idx,
        ) = load_image_paths(data_path, self.selected_view, demo_list)
    ...
    def get_nn_action(self, input_dict):
        ...
        nn_actions, nn_idxs = self.nn_extractor.get_k_nearest_neighbors(
            representation, 10
        )  # 10

        # Appending the item in the buffer and choosing the idx
        choosen_idx = self.buffer.choose(nn_idxs)
        choosen_nn_idx, applied_action = (
            nn_idxs[choosen_idx].item(),
            nn_actions[choosen_idx],
        )

        # Getting the NN idx for visualization
        nn_traj_idx, nn_traj_image_idx = get_traj_state_idxs(
            choosen_nn_idx, self.traj_idx, self.cumm_len
        )
    ...
    def get_action(self, input_dict):
        action, input_nn_image_path, output_nn_image_path = self.get_nn_action(
            input_dict
        )

        nn_image, output_image = cv2.imread(input_nn_image_path), cv2.imread(
            output_nn_image_path
        )
        ...
        cv2.imwrite(
            os.path.join(self.run_store_path, "{}.PNG".format(self.state_counter)),
            display_images,
        )
        self.state_counter += 1
