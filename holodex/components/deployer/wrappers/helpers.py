...
def load_encoder_representations(
    data_path, encoder, selected_view, transform, demo_list=None
):
    transform_dict = {"color_image": [transform], "depth_image": []}
    ...
    torch.save(
        input_representations, os.path.join(data_path, "input_representations.pt")
    )
...
def process_data(data, input_type):
    ...
    processed_hand_abs_joint = converter.scale_transform(
        hand_tensor, HAND_JOINT_LOWER_LIMIT, HAND_JOINT_UPPER_LIMIT
    )
    ...
    if input_type == "ee_pose":
        ...
        processed_arm_data = converter.scale_transform(
            processed_arm_data, ARM_JOINT_LOWER_LIMIT, ARM_JOINT_UPPER_LIMIT
        )
    else:
        raise ValueError(
            f"Invalid input type: {input_type}. Choose either 'ee_pose' or 'joint'."
        )
...
def process_arm_ee_pose(arm_tensor):
    ...
    if ORI_TYPE == "euler_pose":
        ...
        for idx, euler in enumerate(arm_orientation_eulers):
            quat = torch.tensor(
                R.from_euler("xyz", euler.cpu().numpy(), degrees=False).as_quat(),
                device="cuda:0",
            )
            if idx != 0:
...
def load_image_paths(data_path, selected_view, demo_list=None):
    ...
    for idx, demo in enumerate(demo_list):
        image_names = os.listdir(
            os.path.join(
                images_path, demo, "camera_{}_color_image".format(selected_view)
            )
        )
        ...
        demo_input_image_paths = [
            os.path.join(
                images_path,
                demo,
                "camera_{}_color_image".format(selected_view),
                image_names[image_num],
            )
            for image_num in range(len(image_names) - 1)
        ]
        demo_output_image_paths = [
            os.path.join(
                images_path,
                demo,
                "camera_{}_color_image".format(selected_view),
                image_names[image_num + 1],
            )
            for image_num in range(len(image_names) - 1)
        ]
